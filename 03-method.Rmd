...
```{r}
serie_tiempo <- ts(data_sitio$PAYLOAD_4G, start=c(as.numeric(format(fecha_inicio, "%Y")), as.numeric(format(fecha_inicio, "%j"))), frequency=7)
```
...
2. **Tred:** En la tendencia nos muestra la tendencia eliminando el ruido y estacionalidad sin embargo se observan picosal fina.l
  
3. **Seasonal:**: En la Estacionalidad podemos observar un patrón claro y repetitivo , lo cual indica  que existe estacionalidad en los datos en la serie de tiempo.
