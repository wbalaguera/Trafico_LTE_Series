...
Se realiza la prueba de Ljung-Box con un mayor de retrasos de 30 podria
...
original **Media Móvil**: Indica que con 2 medias móviles seria
...
```{r ph_df}
df <- data_sitio[, c("M_FECHA_DIA", "PAYLOAD_4G","FIESTA")]
head(df,5)
```
...
Se crea un nuevo dataframe "holydays" para los días de ferias y fiestas de nuestro proyecto, este se usa para mejorar el ajuste del proyecto 
...
```{r ph_modelo}
modelo_prophet <- prophet(holidays = holidays, daily.seasonality = FALSE,weekly.seasonality=TRUE)

modelo_prophet <- fit.prophet(modelo_prophet, df)
...
head(pronostico,5)
```
**Linea Azul:**: prediccion de Payload 4G, la cual nos muestra una tendencia ascendente hasta 2023 , despues de este año se estabiliza
...
**R² (Coeficiente de Determinación):** este Indica que el modelo explica el 6%  de la variabilidad  en los datos 
...
**MAPE (Mean Absolute Percentage Error:)**: Este indica que el error promedio es del 31.5%, proporciOna una medida relativa del error respecto a los  valores reales
...
### Conclusiones 
Las métricas de error (MAE,MSE,RMSE,R², MAPE) muestran que modelo NO captura tendencias y patrones de tráfico "Payload 4G". La incorporación de las festividades en "Holidays" no ha mejorado la precisión del modelo
...
Convertimos los datos a un objeto tsibble, usando como indice la fecha "ds"
...
report(fit)
```
ARIMA(0,1,2): No hay terminos autoregresivos(AR=0) ,se realiza una diferencia (I=1), y hay dos medias moviles (MA=2).
...
futuro <- new_data(df_tsibble, 365) %>%
  mutate(FIESTA = ifelse(format(ds, "%m-%d") %in% format(fechas_festividades_ultimo_anio, "%m-%d"), 1, 0))

```


Realizamos la prediccion teniendo encuenta los días de fiesta

```{r rs_predic}
predicciones <- fit %>%
  forecast(new_data = futuro)
...
no existe autocorrelaciones significativas en los residuos del modelo ,
esto significa que los residuos no son completamente aleatorios por lo
tanto el modelo puede no estar capturando toda la estructura dependiente en los datos

### Conclusiones

Aunque el modelo ARIMA con la variable de regrsion "FIESTA" , la evidencia de autocorrelación residual y la periocidad semana sugiere realizar ajustes en el modelo para obtener predicciones más precisas
