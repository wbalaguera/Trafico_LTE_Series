...
```{r ph_modelo}
modelo_prophet <- prophet(holidays = holidays, daily.seasonality = FALSE)
...
**Weekly:**Muestra como en los días viernes sabado y domingo aumenta el tráfico, mientras que en los días lunes es mas bajo.

**Yearly** Este gráfico muestra que hay mayor tráfico  los meses de Julio y Diciembre, mientras que Enero y Octubre tienen menor tráfico
...
**MAE (Mean Absolute Error):** Esto significa que en promedio las predicciones estan con una difrenciade 59 unidades 
...
**R² (Coeficiente de Determinación):** este Indica que el modelo explica el 86.6%  de la variabilidad  en los datos 
...
**MAPE (Mean Absolute Percentage Error:)**: Este indica que el error promedio es del 7.2%, proporciOna una medida relativa del error respecto a los  valores reales
...
### Conclusiones 
Las métricas de error (MAE,MSE,RMSE,R², MAPE) muestran que modelo captura tendencias y patrones de tráfico "Payload 4G". La incorporación de las festividades en "Holidays" ha mejorado la precisión del modelo permitiendo ajusta el tráfico duarante estos dias feriados
